    ...
    <div className="flex-1 min-h-screen bg-gray-950 p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">{employees.length} total · {employees.filter((e) => e.status === "active").length} active</p>
          </div>
        </div>
        <button
          onClick={openCreate}
      ...
      <div className="flex gap-3 mb-5">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-gray-900 border border-white/10 rounded-lg pl-9 pr-3 py-2 text-white text-sm placeholder:text-gray-600 focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="bg-gray-900 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
        >
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((emp, i) => (
                <tr
                  key={emp.id}
                  className={`border-b border-white/5 hover:bg-white/[0.02] ${i === filtered.length - 1 ? "border-b-0" : ""}`}
                >
                  <td className="px-4 py-3">
                    <Link href={`/dashboard/employees/${emp.id}`} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
                      <div className="w-8 h-8 rounded-lg bg-blue-600/20 flex items-center justify-center flex-shrink-0">
                        <span className="text-blue-400 font-bold text-sm">{emp.name.charAt(0)}</span>
                      </div>
                      <span className="text-white font-medium">{emp.name}</span>
                    </Link>
                  </td>
                  <td className="px-4 py-3 font-mono text-gray-300">{emp.employeeId}</td>
                  <td className="px-4 py-3 text-gray-400">{emp.department ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-400">{emp.designation ?? "—"}</td>
                  <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-gray-900 rounded-2xl border border-white/10 w-full max-w-lg shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
              <h2 className="text-white font-semibold">{editTarget ? "Edit Employee" : "Add Employee"}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider font-medium mb-1.5 block">Employee ID *</label>
                  <input
                    value={form.employeeId}
                    onChange={(e) => setForm({ ...form, employeeId: e.target.value })}
                    placeholder="e.g. EMP001"
                    disabled={!!editTarget}
                    className="w-full bg-gray-800 border border-white/10 rounded-lg px-3 py-2 text-white text-sm font-mono focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-50"
                  />
                </div>
                <div>
                ...
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider font-medium mb-1.5 block">Phone</label>
                  <input
                    value={form.phone}
                ...
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider font-medium mb-1.5 block">Department</label>
                  <select
                ...
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider font-medium mb-1.5 block">Designation</label>
                  <input
                ...
                <div>
                  <label className="text-xs text-gray-400 uppercase tracking-wider font-medium mb-1.5 block">Joining Date</label>
                  <input
                    type="date"
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-white/10">
              <button onClick={() => setShowModal(false)} className="px-4 py-2 rounded-lg bg-gray-800 hover:bg-gray-700 text-white text-sm transition-colors">
                Cancel
              </button>
              <button
                onClick={handleSave}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60">
          <div className="bg-gray-900 rounded-2xl border border-red-500/20 p-6 w-full max-w-sm shadow-2xl text-center">
            <Trash2 className="w-10 h-10 text-red-400 mx-auto mb-3" />
            <h3 className="text-white font-semibold mb-1">Delete Employee?</h3>
            <p className="text-gray-400 text-sm mb-5">This will permanently delete the employee and all their time logs.</p>
