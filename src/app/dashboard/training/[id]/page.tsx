...
export default function CourseDetailPage() {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      <Link href="/dashboard/training" className="inline-flex items-center gap-1.5 text-sm text-gray-600 hover:text-gray-900 mb-4"><ArrowLeft className="h-4 w-4" /> Training</Link>

      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">{course.title}</h1>
          <p className="text-sm text-gray-600 mt-1">{course.category ?? "Uncategorized"}{course.description ? ` · ${course.description}` : ""}</p>
        </div>
        <button onClick={togglePublish} className={`px-3 py-1.5 text-xs font-medium rounded-md ${course.isPublished ? "bg-green-600/20 text-green-600" : "bg-blue-600 text-white hover:bg-blue-500"}`}>{course.isPublished ? "Published" : "Publish"}</button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div>
          <div className="flex items-center justify-between mb-2">
            <h2 className="flex items-center gap-1.5 text-sm font-semibold text-gray-700"><BookOpen className="h-4 w-4" /> Lessons</h2>
            <button onClick={() => { setError(""); setShowLesson(true); }} className="text-xs text-blue-600 hover:text-blue-600">+ Add lesson</button>
          </div>
          <div className="space-y-1">
            {course.lessons.length === 0 ? <p className="text-xs text-gray-600">No lessons yet.</p> : course.lessons.map((l, i) => (
              <div key={l.id} className="bg-white border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-700">{i + 1}. {l.title} <span className="text-xs text-gray-600">· {l.contentType}</span></div>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            {enrollments.length === 0 ? <p className="text-xs text-gray-600">No one enrolled.</p> : enrollments.map((e) => (
              <div key={e.id} className="bg-white border border-gray-200 rounded-md">
                <button onClick={() => setOpenEnrollment(openEnrollment === e.id ? null : e.id)} className="w-full flex items-center justify-between px-3 py-2 text-sm">
                  <span className="text-gray-900">{e.employeeName}</span>
                  <span className="flex items-center gap-2">
                    <span className="w-20 h-1.5 bg-gray-100 rounded-full overflow-hidden"><span className="block h-full bg-blue-500" style={{ width: `${e.progressPct}%` }} /></span>
                  ...
                  <div className="px-3 pb-2 space-y-1">
                    {course.lessons.map((l) => (
                      <div key={l.id} className="flex items-center justify-between text-xs">
                        ...
                        <button onClick={() => complete(e.id, l.id)} className="inline-flex items-center gap-1 text-gray-500 hover:text-green-600"><Check className="h-3.5 w-3.5" /> Complete</button>
                      </div>
                    ))}
                  </div>
                )}
                {openEnrollment === e.id && e.status === "completed" && <div className="px-3 pb-2 text-xs text-green-600">Completed ✓</div>}
              </div>
            ))}
          </div>
        </div>
      ...
      {showLesson && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowLesson(false)}>
          <div className="bg-white border border-gray-200 rounded-lg w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold text-gray-900">Add Lesson</h2><button onClick={() => setShowLesson(false)} className="text-gray-500 hover:text-gray-700"><X className="h-5 w-5" /></button></div>
            <div className="space-y-3">
              ...
              {lessonForm.contentType === "text"
                ...
                : <label className="block"><span className="text-xs text-gray-500">URL</span><input value={lessonForm.contentUrl} onChange={(e) => setLessonForm({ ...lessonForm, contentUrl: e.target.value })} className="mt-1 w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-900" /></label>}
            </div>
            <div className="flex justify-end gap-2 mt-5"><button onClick={() => setShowLesson(false)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Cancel</button><button onClick={addLesson} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-md">Add</button></div>
          </div>
        </div>
      )}
    </div>
