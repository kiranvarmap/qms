  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3"><GraduationCap className="h-6 w-6 text-blue-400" /><h1 className="text-xl font-semibold text-white">Training</h1></div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="col-span-2">
          <h2 className="text-sm font-semibold text-gray-300 mb-2">Courses</h2>
          <div className="grid grid-cols-2 gap-3">
            {loading ? <p className="text-gray-500 text-sm">Loading…</p> : courses.length === 0 ? <p className="text-gray-500 text-sm">No courses yet.</p> : courses.map((c) => (
              <Link key={c.id} href={`/dashboard/training/${c.id}`} className="block bg-gray-900 border border-white/10 rounded-lg p-4 hover:border-white/20">
                <div className="flex items-start justify-between">
                  <h3 className="text-gray-200 font-medium">{c.title}</h3>
                  {!c.isPublished && <span className="text-xs text-amber-400">draft</span>}
                </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h2 className="flex items-center gap-1.5 text-sm font-semibold text-gray-300"><Award className="h-4 w-4" /> Certifications</h2>
            <button onClick={() => { setError(""); setShowCert(true); }} className="text-xs text-blue-400 hover:text-blue-300">+ Define</button>
          </div>
          <div className="space-y-1 mb-4">
            {certs.map((c) => <div key={c.id} className="text-sm text-gray-400 bg-gray-900 border border-white/10 rounded-md px-3 py-2">{c.name} <span className="text-gray-600">· {c.validityMonths ? `${c.validityMonths}mo` : "no expiry"}</span></div>)}
            {certs.length === 0 && <p className="text-xs text-gray-600">None defined.</p>}
          </div>
          <h3 className="text-xs font-semibold text-gray-400 mb-2">Register</h3>
          <div className="space-y-1">
            {records.length === 0 ? <p className="text-xs text-gray-600">No certifications issued.</p> : records.map((r) => (
              <div key={r.id} className="flex items-center justify-between text-xs bg-gray-900 border border-white/10 rounded-md px-3 py-2">
                <span className="text-gray-300 truncate">{r.employeeName} · {r.certificationName}</span>
      ...
      {showCourse && (
        <Modal title="New Course" onClose={() => setShowCourse(false)} error={error}>
          <div className="space-y-3">
            ...
            <label className="block"><span className="text-xs text-gray-500">Description</span><textarea value={courseForm.description} onChange={(e) => setCourseForm({ ...courseForm, description: e.target.value })} rows={2} className="mt-1 w-full bg-gray-950 border border-white/10 rounded-md px-3 py-2 text-sm text-gray-200" /></label>
          </div>
        ...
        <Modal title="Define Certification" onClose={() => setShowCert(false)} error={error}>
          <div className="space-y-3">
            ...
            <label className="block"><span className="text-xs text-gray-500">Auto-issue on course completion</span>
              <select value={certForm.requiresCourseId} onChange={(e) => setCertForm({ ...certForm, requiresCourseId: e.target.value })} className="mt-1 w-full bg-gray-950 border border-white/10 rounded-md px-3 py-2 text-sm text-gray-200"><option value="">None</option>{courses.map((c) => <option key={c.id} value={c.id}>{c.title}</option>)}</select>
            </label>
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-gray-900 border border-white/10 rounded-lg w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold text-white">{title}</h2><button onClick={onClose} className="text-gray-500 hover:text-gray-300"><X className="h-5 w-5" /></button></div>
...
function Actions({ onCancel, onSave }: { onCancel: () => void; onSave: () => void }) {
  return <div className="flex justify-end gap-2 mt-5"><button onClick={onCancel} className="px-4 py-2 text-sm text-gray-400 hover:text-gray-200">Cancel</button><button onClick={onSave} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-md">Save</button></div>;
}
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return <label className="block"><span className="text-xs text-gray-500">{label}</span><input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full bg-gray-950 border border-white/10 rounded-md px-3 py-2 text-sm text-gray-200" /></label>;
}
