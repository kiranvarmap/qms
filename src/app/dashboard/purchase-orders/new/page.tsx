...
interface Product { id: string; name: string; costMinor?: number; priceMinor?: number }
interface TaxRate { id: string; name: string; rateBasisPoints: number }
interface Line { productId: string; description: string; quantity: string; unitCost: string; taxRateId: string }

const inp = "w-full bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-gray-900 focus:border-blue-500";
const emptyLine: Line = { productId: "", description: "", quantity: "1", unitCost: "0", taxRateId: "" };
const PAYMENT_TERMS = [
...
export default function NewPurchaseOrderPage() {
  ...
  const [h, setH] = useState({
    vendorId: "", reference: "", orderDate: new Date().toISOString().slice(0, 10), expectedDate: "",
    paymentTermsLabel: "due_on_receipt", shipmentPreference: "", reverseCharge: false,
    deliveryAddressType: "organization", deliveryCustomerId: "",
    discountValue: "0", discountType: "percent", withholdingType: "", withholdingTaxRateId: "",
    adjustment: "0", notes: "", termsConditions: "",
  });
  const setF = (k: keyof typeof h, v: string | boolean) => setH((p) => ({ ...p, [k]: v }));
  ...
  useEffect(() => {
    ...
    Promise.all([
      g(`/api/vendors?workspaceId=${workspaceId}&status=active`), g(`/api/customers?workspaceId=${workspaceId}`),
      g(`/api/products?workspaceId=${workspaceId}`), g(`/api/tax-rates?workspaceId=${workspaceId}`),
    ]).then(([ve, cu, pr, tx]) => { setVendors(ve); setCustomers(cu); setProducts(pr); setTaxRates(tx); });
  }, [workspaceId]);
  ...
  const totals = useMemo(() => {
    ...
    const tax = lines.reduce((s, l) => s + (Number(l.quantity) || 0) * (Number(l.unitCost) || 0) * (taxBp(l.taxRateId) / 10000), 0);
  ...
  const save = async () => {
    if (!h.vendorId) { setError("Vendor is required"); return; }
    const payloadLines = lines.filter((l) => l.description.trim() || l.productId).map((l) => ({
      ...
      quantity: Number(l.quantity) || 1, unitCost: Number(l.unitCost) || 0, taxRateId: l.taxRateId || undefined,
    }));
    if (payloadLines.length === 0) { setError("Add at least one line item"); return; }
    setSaving(true); setError("");
    const res = await fetch("/api/purchase-orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      workspaceId, vendorId: h.vendorId,
      reference: h.reference || undefined,
      orderDate: h.orderDate ? new Date(h.orderDate).toISOString() : undefined,
      expectedDate: h.expectedDate ? new Date(h.expectedDate).toISOString() : undefined,
      paymentTermsLabel: h.paymentTermsLabel, shipmentPreference: h.shipmentPreference || undefined,
      reverseCharge: h.reverseCharge,
      deliveryAddressType: h.deliveryAddressType, deliveryCustomerId: h.deliveryCustomerId || undefined,
      ...
      notes: h.notes || undefined, termsConditions: h.termsConditions || undefined,
      lines: payloadLines,
    }) });
    setSaving(false);
    if (!res.ok) { const e = await res.json().catch(() => ({})); setError(e.error || "Failed to save"); return; }
    const po = await res.json();
    if (po?.id) router.push(`/dashboard/purchase-orders/${po.id}`);
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto pb-28">
      <Link href="/dashboard/purchase-orders" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 mb-3"><ArrowLeft className="h-4 w-4" /> Purchase Orders</Link>
      <h1 className="text-[24px] font-semibold tracking-tight text-gray-900 [font-family:var(--font-display)] mb-6">New Purchase Order</h1>
      ...
      <div className="grid grid-cols-2 gap-x-10 gap-y-4 max-w-4xl">
        <Row label="Vendor Name" required>
          <NativeSelect value={h.vendorId} onChange={(e) => setF("vendorId", e.target.value)} className={inp}><option value="">Select a Vendor</option>{vendors.map((v) => <option key={v.id} value={v.id}>{v.name}</option>)}</NativeSelect>
        </Row>
        <div />
        <Row label="Delivery Address" required>
          <div className="flex items-center gap-6 text-sm text-gray-900 pt-1">
            <label className="inline-flex items-center gap-2"><input type="radio" checked={h.deliveryAddressType === "organization"} onChange={() => setF("deliveryAddressType", "organization")} /> Organization</label>
            <label className="inline-flex items-center gap-2"><input type="radio" checked={h.deliveryAddressType === "customer"} onChange={() => setF("deliveryAddressType", "customer")} /> Customer</label>
          </div>
        </Row>
        {h.deliveryAddressType === "customer"
          ? <Row label="Deliver to Customer"><NativeSelect value={h.deliveryCustomerId} onChange={(e) => setF("deliveryCustomerId", e.target.value)} className={inp}><option value="">Select a customer</option>{customers.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</NativeSelect></Row>
          : <div />}
        <Row label="Reference#"><input value={h.reference} onChange={(e) => setF("reference", e.target.value)} className={inp} /></Row>
        <div />
        <Row label="Date"><DateInput value={h.orderDate} onChange={(e) => setF("orderDate", e.target.value)} /></Row>
        <Row label="Delivery Date"><DateInput value={h.expectedDate} onChange={(e) => setF("expectedDate", e.target.value)} /></Row>
        <Row label="Payment Terms"><NativeSelect value={h.paymentTermsLabel} onChange={(e) => setF("paymentTermsLabel", e.target.value)} className={inp}>{PAYMENT_TERMS.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</NativeSelect></Row>
        <Row label="Shipment Preference"><input value={h.shipmentPreference} onChange={(e) => setF("shipmentPreference", e.target.value)} placeholder="Choose the shipment preference" className={inp} /></Row>
        <Row label="">
          <label className="inline-flex items-center gap-2 text-sm text-gray-700 pt-1"><input type="checkbox" checked={h.reverseCharge} onChange={(e) => setF("reverseCharge", e.target.checked)} /> This transaction is applicable for reverse charge</label>
        </Row>
      </div>
              ...
              return (
                <tr key={i} className="border-b border-gray-100 align-top">
                  <td className="px-2 py-2">
                    <NativeSelect value={l.productId} onChange={(e) => { const p = products.find((x) => x.id === e.target.value); setLines(lines.map((x, j) => j === i ? { ...x, productId: e.target.value, description: p?.name ?? x.description, unitCost: p?.costMinor != null ? String(p.costMinor / 100) : x.unitCost } : x)); }} className={`${inp} mb-1`}><option value="">Type or click to select an item</option>{products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</NativeSelect>
                  ...
                  <td className="px-2 py-2"><input type="number" value={l.unitCost} onChange={(e) => setLines(lines.map((x, j) => j === i ? { ...x, unitCost: e.target.value } : x))} className={`${inp} text-right`} /></td>
      ...
      <div className="mt-6 grid grid-cols-2 gap-10">
        <div className="space-y-5">
          <div><label className="text-sm text-gray-700">Notes</label><textarea value={h.notes} onChange={(e) => setF("notes", e.target.value)} rows={3} placeholder="Will be displayed on purchase order" className={`${inp} mt-1`} /></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-3 flex gap-2 z-10">
        <button onClick={save} disabled={saving || !workspaceId} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-medium rounded-md">{saving ? "Saving…" : "Save Purchase Order"}</button>
        <Link href="/dashboard/purchase-orders" className="px-5 py-2 text-gray-600 hover:text-gray-900 text-sm font-medium">Cancel</Link>
      </div>
    </div>
  );
}
