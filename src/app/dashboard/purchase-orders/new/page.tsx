    ...
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <label className="block">
          ...
          <select value={vendorId} onChange={(e) => setVendorId(e.target.value)} className="mt-1 w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-900">
        ...
        <label className="block">
          ...
          <input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} className="mt-1 w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-900" />
        </label>
      </div>
            ...
            {lines.map((l, i) => (
              <tr key={i} className="border-b border-gray-200">
                <td className="px-3 py-2">
                  <input value={l.description} onChange={(e) => setLine(i, { description: e.target.value })} placeholder="Item or service" className="w-full bg-transparent text-gray-900 outline-none" />
                </td>
                <td className="px-3 py-2">
                  <input type="number" value={l.quantity} onChange={(e) => setLine(i, { quantity: e.target.value })} className="w-full bg-transparent text-gray-900 outline-none" />
                </td>
                <td className="px-3 py-2">
                  <input type="number" step="0.01" value={l.unitCost} onChange={(e) => setLine(i, { unitCost: e.target.value })} className="w-full bg-transparent text-gray-900 outline-none" />
                </td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2 text-right">
                  <button onClick={() => removeLine(i)} className="p-1 text-gray-500 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={addLine} className="inline-flex items-center gap-1.5 text-sm text-blue-600 hover:text-blue-600">
          <Plus className="h-4 w-4" /> Add line
        </button>
        <div className="text-sm text-gray-600">Estimated total: <span className="text-gray-900 font-medium">${grandTotal.toFixed(2)}</span></div>
      </div>
      ...
      <div className="flex justify-end gap-2">
        <Link href="/dashboard/purchase-orders" className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Cancel</Link>
