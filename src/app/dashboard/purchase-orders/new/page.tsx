...
interface Line { description: string; quantity: string; unitCost: string; taxRateId: string }

const emptyLine: Line = { description: "", quantity: "1", unitCost: "0", taxRateId: "" };
...
export default function NewPurchaseOrderPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <h1 className="text-xl font-semibold text-white mb-6">New Purchase Order</h1>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <label className="block">
          ...
          <select value={vendorId} onChange={(e) => setVendorId(e.target.value)} className="mt-1 w-full bg-gray-950 border border-white/10 rounded-md px-3 py-2 text-sm text-gray-200">
        ...
        <label className="block">
          ...
          <input type="date" value={expectedDate} onChange={(e) => setExpectedDate(e.target.value)} className="mt-1 w-full bg-gray-950 border border-white/10 rounded-md px-3 py-2 text-sm text-gray-200" />
        </label>
      </div>
            ...
            {lines.map((l, i) => (
              <tr key={i} className="border-b border-white/5">
                <td className="px-3 py-2">
                  <input value={l.description} onChange={(e) => setLine(i, { description: e.target.value })} placeholder="Item or service" className="w-full bg-transparent text-gray-200 outline-none" />
                </td>
                <td className="px-3 py-2">
                  <input type="number" value={l.quantity} onChange={(e) => setLine(i, { quantity: e.target.value })} className="w-full bg-transparent text-gray-200 outline-none" />
                </td>
                <td className="px-3 py-2">
                  <input type="number" step="0.01" value={l.unitCost} onChange={(e) => setLine(i, { unitCost: e.target.value })} className="w-full bg-transparent text-gray-200 outline-none" />
                </td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2 text-right">
                  <button onClick={() => removeLine(i)} className="p-1 text-gray-500 hover:text-red-400"><Trash2 className="h-4 w-4" /></button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={addLine} className="inline-flex items-center gap-1.5 text-sm text-blue-400 hover:text-blue-300">
          <Plus className="h-4 w-4" /> Add line
        </button>
        <div className="text-sm text-gray-400">Estimated total: <span className="text-gray-100 font-medium">${grandTotal.toFixed(2)}</span></div>
      </div>
      ...
      <div className="flex justify-end gap-2">
        <Link href="/dashboard/purchase-orders" className="px-4 py-2 text-sm text-gray-400 hover:text-gray-200">Cancel</Link>
        <button onClick={save} disabled={saving || !workspaceId} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-sm font-medium rounded-md">
