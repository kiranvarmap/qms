...
export default function NewSalesOrderPage() {
  ...
  const [h, setH] = useState({
    customerId: "", reference: "", orderDate: new Date().toISOString().slice(0, 10),
    salespersonEmployeeId: "", projectId: "", warehouseId: "", subject: "",
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      g(`/api/emp-projects?workspaceId=${workspaceId}`), g(`/api/warehouses?workspaceId=${workspaceId}`),
    ]).then(([cu, pr, tx, em, pj, wh]) => { setCustomers(cu); setProducts(pr); setTaxRates(tx); setEmployees(em); setProjects(pj); setWarehouses(wh); });
  }, [workspaceId]);
  ...
  const save = async () => {
    ...
    const res = await fetch("/api/sales-orders", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
    ...
    <div className="p-8 max-w-6xl mx-auto pb-28">
      ...
      <div className="mt-6 grid grid-cols-2 gap-10">
        <div className="space-y-5">
          <div><label className="text-sm text-gray-700">Customer Notes</label><textarea value={h.customerNotes} onChange={(e) => setF("customerNotes", e.target.value)} rows={3} placeholder="Will be displayed on the sales order" className={`${inp} mt-1`} /></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-3 flex gap-2 z-10">
        <button onClick={save} disabled={saving || !workspaceId} className="px-5 py-2 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white text-sm font-medium rounded-md">{saving ? "Saving…" : "Save Sales Order"}</button>
        <Link href="/dashboard/sales-orders" className="px-5 py-2 text-gray-600 hover:text-gray-900 text-sm font-medium">Cancel</Link>
      </div>
    </div>
  );
}
