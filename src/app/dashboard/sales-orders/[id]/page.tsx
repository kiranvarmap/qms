...
const statusColors: Record<string, string> = {
  draft: "bg-gray-600/40 text-gray-300", pending_approval: "bg-yellow-500/20 text-yellow-400",
  approved: "bg-blue-500/20 text-blue-400", reserved: "bg-cyan-500/20 text-cyan-400",
  picking: "bg-amber-500/20 text-amber-400", packed: "bg-amber-500/20 text-amber-300",
  shipped: "bg-indigo-500/20 text-indigo-400", delivered: "bg-green-500/20 text-green-400",
  invoiced: "bg-purple-500/20 text-purple-400", cancelled: "bg-red-500/20 text-red-400",
};
...
export default function SalesOrderDetailPage() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <Link href="/dashboard/sales-orders" className="inline-flex items-center gap-1.5 text-sm text-gray-400 hover:text-gray-200 mb-4">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-semibold text-white">{so.docNumber}</h1>
            <span className={`px-2 py-0.5 rounded text-xs font-medium ${statusColors[so.status] ?? ""}`}>{so.status.replace(/_/g, " ")}</span>
          </div>
          <p className="text-sm text-gray-400 mt-1">{so.customer?.name ?? "—"}</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {canCancel && <button onClick={() => soAct("cancel")} disabled={busy} className="inline-flex items-center gap-1.5 px-3 py-1.5 border border-white/10 text-gray-300 hover:text-white text-xs font-medium rounded-md"><XCircle className="h-4 w-4" /> Cancel</button>}
        </div>
      </div>
          ...
          <tbody>
            {so.lines.map((l) => (
              <tr key={l.id} className="border-b border-white/5">
                ...
                <td className="px-4 py-2.5 text-right text-cyan-400">{l.qtyReserved}</td>
                <td className="px-4 py-2.5 text-right text-indigo-400">{l.qtyShipped}</td>
                <td className="px-4 py-2.5 text-right text-gray-200">{money(l.amountMinor + l.lineTaxMinor, so.currency)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="flex justify-end mb-8">
        <div className="w-56 text-sm space-y-1">
          <div className="flex justify-between text-gray-400"><span>Subtotal</span><span>{money(so.subtotalMinor, so.currency)}</span></div>
          <div className="flex justify-between text-gray-400"><span>Tax</span><span>{money(so.taxMinor, so.currency)}</span></div>
          <div className="flex justify-between text-gray-100 font-medium border-t border-white/10 pt-1"><span>Total</span><span>{money(so.totalMinor, so.currency)}</span></div>
        </div>
      </div>

      {so.shipments.length > 0 && (
        <div className="mb-8">
          <h2 className="text-sm font-semibold text-gray-300 mb-2">Shipments</h2>
          <div className="space-y-1">
            {so.shipments.map((s) => {
              const next = NEXT_ACTION[s.status];
              return (
                <div key={s.id} className="bg-gray-900 border border-white/10 rounded-md px-3 py-2 text-sm flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${statusColors[s.status] ?? "bg-gray-700 text-gray-300"}`}>{s.status}</span>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowShip(false)}>
          <div className="bg-gray-900 border border-white/10 rounded-lg w-full max-w-lg p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-white">New Shipment</h2>
              <button onClick={() => setShowShip(false)} className="text-gray-500 hover:text-gray-300"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-2">
              {so.lines.filter((l) => l.qtyShipped < l.quantity).map((l) => (
                <div key={l.id} className="flex items-center justify-between gap-3">
                  ...
                  <input type="number" min="0" max={l.quantity - l.qtyShipped} placeholder="0" value={shipQty[l.id] ?? ""} onChange={(e) => setShipQty((q) => ({ ...q, [l.id]: e.target.value }))} className="w-24 bg-gray-950 border border-white/10 rounded-md px-2 py-1.5 text-sm text-gray-200" />
                </div>
              ))}
            </div>
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowShip(false)} className="px-4 py-2 text-sm text-gray-400 hover:text-gray-200">Cancel</button>
