...
import { Add as Plus, Group as Users2, Key as KeyRound, CloseSmall as X, Alert as AlertCircle } from "@vibe/icons";
            ...
            {loading ? (
              ...
              <tr key={c.id} className="border-b border-gray-200 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">
                  <button onClick={() => openPortal(c)} className="inline-flex items-center gap-1.5 px-2.5 py-1 border border-gray-200 text-gray-700 hover:text-gray-900 text-xs rounded-md"><KeyRound className="h-3.5 w-3.5" /> Manage access</button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="mb-4">
            {contacts.length === 0 ? (
              ...
              <div className="space-y-1">
                {contacts.map((ct) => (
                  <div key={ct.id} className="flex justify-between text-sm bg-gray-50 rounded-md px-3 py-2">
                    <span className="text-gray-900">{ct.name} <span className="text-gray-500">· {ct.email}</span></span>
...
function Actions({ saving, onCancel, onSave, label }: { saving: boolean; onCancel: () => void; onSave: () => void; label: string }) {
  return (
    <div className="flex justify-end gap-2 mt-5">
      <button onClick={onCancel} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Close</button>
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <label className="block">
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-900" />
    </label>
  );
}
