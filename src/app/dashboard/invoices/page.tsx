...
const statusColors: Record<string, string> = {
  ...
  void: "bg-gray-700/60 text-gray-500",
};
          ...
          <tbody>
            {loading ? (
              ...
              <tr key={inv.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                <td className="px-4 py-3"><Link href={`/dashboard/invoices/${inv.id}`} className="text-blue-400 hover:underline font-medium">{inv.docNumber}</Link></td>
                <td className="px-4 py-3 text-gray-300">{inv.customerName ?? "—"}</td>
                ...
                <td className="px-4 py-3 text-right text-gray-200">{money(inv.totalMinor, inv.currency)}</td>
                <td className="px-4 py-3 text-right text-gray-300">{money(inv.totalMinor - inv.amountPaidMinor, inv.currency)}</td>
