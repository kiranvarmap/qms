    ...
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <Link href="/dashboard/inventory/warehouses" className="inline-flex items-center gap-1.5 px-3 py-2 text-sm text-gray-700 hover:text-gray-900 border border-gray-200 rounded-md">
      ...
      {warehouses.length === 0 && (
        <div className="mb-4 flex items-center gap-2 text-sm text-amber-600 bg-amber-500/10 rounded-md px-3 py-2">
              ...
              return (
                <tr key={p.id} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="px-4 py-3">
                    <Link href={`/dashboard/inventory/${p.id}`} className="text-blue-600 hover:underline font-medium">{p.name}</Link>
                    {p.type === "service" && <span className="ml-2 text-xs text-gray-500">service</span>}
                  </td>
                  <td className="px-4 py-3 text-gray-600">{p.sku || "—"}</td>
                  <td className="px-4 py-3 text-right text-gray-700">{money(p.priceMinor)}</td>
                  <td className="px-4 py-3 text-right text-gray-700">{p.trackInventory ? p.onHand : "—"}</td>
                  <td className="px-4 py-3 text-right text-gray-600">{p.trackInventory ? p.committed : "—"}</td>
                  <td className="px-4 py-3 text-right">
                    {p.trackInventory ? (
                      <span className={low ? "text-amber-600 font-medium inline-flex items-center gap-1" : "text-gray-900"}>
                  ...
                  <td className="px-4 py-3 text-right">
                    {p.trackInventory && (
                      <button onClick={() => openAdjust(p)} className="p-1.5 text-gray-500 hover:text-gray-900" title="Adjust stock"><SlidersHorizontal className="h-4 w-4" /></button>
                    )}
                  </td>
                </tr>
              );
...
function Field({ label, value, onChange, type = "text", className = "" }: { label: string; value: string; onChange: (v: string) => void; type?: string; className?: string }) {
  return (
    <label className={`block ${className}`}>
      ...
      <input type={type} step={type === "number" ? "any" : undefined} value={value} onChange={(e) => onChange(e.target.value)} className="mt-1 w-full bg-gray-50 border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-900" />
    </label>
  );
}
