...
const typeColors: Record<string, string> = {
  ...
  shipment: "text-red-400",
  reservation: "text-amber-400",
  reservation_release: "text-blue-400",
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      {product.trackInventory && (
        <div className="mb-8">
          <h2 className="text-sm font-semibold text-gray-300 mb-2">Stock by warehouse</h2>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {product.levels.length === 0 ? (
                  ...
                  <tr key={l.warehouseId} className="border-b border-white/5">
                    <td className="px-4 py-2.5 text-gray-200">{l.warehouseName ?? "—"}</td>
                    <td className="px-4 py-2.5 text-right text-gray-300">{l.onHand}</td>
                    <td className="px-4 py-2.5 text-right text-gray-400">{l.committed}</td>
                    <td className="px-4 py-2.5 text-right text-gray-100">{l.available}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <h2 className="text-sm font-semibold text-gray-300 mb-2">Movement ledger</h2>
      <div className="bg-gray-900 border border-white/10 rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-gray-500 border-b border-white/10">
          ...
          <tbody>
            {movements.length === 0 ? (
              ...
              <tr key={m.id} className="border-b border-white/5">
                <td className="px-4 py-2.5 text-gray-400">{new Date(m.createdAt).toLocaleString()}</td>
                <td className={`px-4 py-2.5 font-medium ${typeColors[m.type] ?? "text-gray-300"}`}>{m.type.replace(/_/g, " ")}</td>
                <td className="px-4 py-2.5 text-gray-400">{m.warehouseName ?? "—"}</td>
                <td className="px-4 py-2.5 text-right text-gray-300">{m.onHandDelta > 0 ? `+${m.onHandDelta}` : m.onHandDelta}</td>
                <td className="px-4 py-2.5 text-right text-gray-400">{m.committedDelta > 0 ? `+${m.committedDelta}` : m.committedDelta || "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
