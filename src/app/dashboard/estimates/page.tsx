            ...
            {loading ? (
              ...
              <tr key={e.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                <td className="px-4 py-3">
                  <Link href={`/dashboard/estimates/${e.id}`} className="text-blue-400 hover:underline font-medium">{e.docNumber}</Link>
                ...
                <td className="px-4 py-3 text-right text-gray-200">{money(e.totalMinor, e.currency)}</td>
                <td className="px-4 py-3 text-gray-400">{e.validUntil ? new Date(e.validUntil).toLocaleDateString() : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
