...
export default function NewQuotePage() {
  ...
  const [h, setH] = useState({
    customerId: "", reference: "", quoteDate: new Date().toISOString().slice(0, 10), expiryDate: "",
  ...
  const save = async (sendAfter: boolean) => {
    ...
    const res = await fetch("/api/estimates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      workspaceId, customerId: h.customerId,
      validUntil: h.expiryDate ? new Date(h.expiryDate).toISOString() : undefined,
    ...
    if (sendAfter && est?.id) await fetch(`/api/estimates/${est.id}/send`, { method: "POST" }).catch(() => {});
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto pb-28">
      <Link href="/dashboard/estimates" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900 mb-3"><ArrowLeft className="h-4 w-4" /> Quotes</Link>
      <h1 className="text-[24px] font-semibold tracking-tight text-gray-900 [font-family:var(--font-display)] mb-6">New Quote</h1>
      ...
      <div className="grid grid-cols-2 gap-x-10 gap-y-4 max-w-4xl">
        ...
        <div />
        <Row label="Quote Date" required><DateInput value={h.quoteDate} onChange={(e) => setF("quoteDate", e.target.value)} /></Row>
        <Row label="Expiry Date"><DateInput value={h.expiryDate} onChange={(e) => setF("expiryDate", e.target.value)} /></Row>
        ...
        <Row label="Subject"><input value={h.subject} onChange={(e) => setF("subject", e.target.value)} placeholder="Let your customer know what this Quote is for" className={inp} /></Row>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-10">
        <div className="space-y-5">
          <div><label className="text-sm text-gray-700">Customer Notes</label><textarea value={h.customerNotes} onChange={(e) => setF("customerNotes", e.target.value)} rows={3} placeholder="Will be displayed on the quote" className={`${inp} mt-1`} /></div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-3 flex gap-2 z-10">
        ...
        <Link href="/dashboard/estimates" className="px-5 py-2 text-gray-600 hover:text-gray-900 text-sm font-medium">Cancel</Link>
      </div>
    </div>
  );
}
