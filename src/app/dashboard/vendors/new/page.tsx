...
export default function NewVendorPage() {
  ...
  const [f, setF] = useState({
    vendorType: "business", salutation: "", firstName: "", lastName: "", companyName: "",
    displayName: "", email: "", workPhone: "", mobile: "", vendorLanguage: "English",
    gstTreatment: "", placeOfSupply: "", gstin: "", pan: "", taxPreference: "taxable",
    currency: "INR", openingBalance: "", paymentTermsLabel: "due_on_receipt", notes: "",
  });
  ...
  const [bank, setBank] = useState({ accountName: "", bankName: "", accountNumber: "", reAccountNumber: "", routing: "", branch: "" });
  const setB = (k: keyof typeof bank, v: string) => setBank((p) => ({ ...p, [k]: v }));
  ...
  const save = async () => {
    ...
    if (bank.accountNumber && bank.accountNumber !== bank.reAccountNumber) { setError("Account numbers do not match"); setTab("bank"); return; }
    ...
    const bankAccounts = (bank.accountNumber || bank.accountName || bank.bankName)
      ? [{ accountName: bank.accountName || undefined, bankName: bank.bankName || undefined, accountNumber: bank.accountNumber || undefined, routing: bank.routing || undefined, branch: bank.branch || undefined }]
      : [];
    const body = {
      workspaceId,
      vendorType: f.vendorType, salutation: f.salutation || undefined, firstName: f.firstName || undefined,
      ...
      vendorLanguage: f.vendorLanguage, gstTreatment: f.gstTreatment || undefined, placeOfSupply: f.placeOfSupply || undefined,
      ...
      paymentTermsLabel: f.paymentTermsLabel,
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto pb-28">
      ...
      <div className="space-y-4">
        <Row label="Vendor Type">
          <div className="flex items-center gap-6 text-sm text-gray-900">
            <label className="inline-flex items-center gap-2"><input type="radio" checked={f.vendorType === "business"} onChange={() => set("vendorType", "business")} /> Business</label>
            <label className="inline-flex items-center gap-2"><input type="radio" checked={f.vendorType === "individual"} onChange={() => set("vendorType", "individual")} /> Individual</label>
          </div>
        </Row>
      ...
      {/* ── Tabs ─────────────────────────────────── */}
      <div className="mt-8 border-b border-gray-200 flex gap-6 flex-wrap">
        <TabBtn id="other" label="Other Details" /><TabBtn id="address" label="Address" /><TabBtn id="contacts" label="Contact Persons" />
        <TabBtn id="bank" label="Bank Details" /><TabBtn id="custom" label="Custom Fields" /><TabBtn id="tags" label="Reporting Tags" /><TabBtn id="remarks" label="Remarks" />
      </div>
      ...
      <div className="py-6">
        ...
        {tab === "bank" && (
          <div className="space-y-4 max-w-2xl">
            <Row label="Account Holder Name"><input value={bank.accountName} onChange={(e) => setB("accountName", e.target.value)} className={inp} /></Row>
            <Row label="Bank Name"><input value={bank.bankName} onChange={(e) => setB("bankName", e.target.value)} className={inp} /></Row>
            <Row label="Account Number"><input value={bank.accountNumber} onChange={(e) => setB("accountNumber", e.target.value)} className={inp} /></Row>
            <Row label="Re-enter Account Number"><input value={bank.reAccountNumber} onChange={(e) => setB("reAccountNumber", e.target.value)} className={inp} /></Row>
            <Row label="IFSC / SWIFT"><input value={bank.routing} onChange={(e) => setB("routing", e.target.value)} className={inp} /></Row>
            <Row label="Branch"><input value={bank.branch} onChange={(e) => setB("branch", e.target.value)} className={inp} /></Row>
          </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-8 py-3 flex gap-2 z-10">
        ...
        <Link href="/dashboard/vendors" className="px-5 py-2 border border-gray-300 text-gray-700 hover:bg-gray-50 text-sm font-medium rounded-md">Cancel</Link>
      </div>
    </div>
  );
}
