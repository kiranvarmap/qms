  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3"><Users className="h-6 w-6 text-blue-600" /><h1 className="text-xl font-semibold text-gray-900">HR Directory</h1></div>
      ...
      <div className="grid grid-cols-3 gap-6">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={emp.id} className="border-b border-gray-200 hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-900">{emp.name}<div className="text-xs text-gray-500">{emp.designation ?? emp.employeeId}</div></td>
                  <td className="px-4 py-3 text-gray-600">{emp.departmentName ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-600">{emp.managerName ?? "—"}</td>
                  <td className="px-4 py-3 text-gray-600">{emp.employmentType?.replace(/_/g, " ") ?? "—"}</td>
                  <td className="px-4 py-3 text-right"><button onClick={() => openEdit(emp)} className="p-1.5 text-gray-500 hover:text-gray-900"><Pencil className="h-4 w-4" /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            ...
            <div className="space-y-1 mb-2">{departments.map((d) => <div key={d.id} className="text-sm text-gray-600">{d.name}</div>)}{departments.length === 0 && <p className="text-xs text-gray-600">None yet.</p>}</div>
            <div className="flex gap-2"><input value={newDept} onChange={(e) => setNewDept(e.target.value)} placeholder="New department" className="flex-1 bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-sm text-gray-900" /><button onClick={addDept} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-md">Add</button></div>
          </div>
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-2">Leave types</h3>
            <div className="space-y-1 mb-2">{leaveTypes.map((l) => <div key={l.id} className="text-sm text-gray-600">{l.name} <span className="text-gray-600">· {l.defaultDays}d</span></div>)}{leaveTypes.length === 0 && <p className="text-xs text-gray-600">None yet.</p>}</div>
            <div className="flex gap-2"><input value={newLeave.name} onChange={(e) => setNewLeave({ ...newLeave, name: e.target.value })} placeholder="Name" className="flex-1 bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-sm text-gray-900" /><input value={newLeave.defaultDays} onChange={(e) => setNewLeave({ ...newLeave, defaultDays: e.target.value })} type="number" placeholder="Days" className="w-16 bg-gray-50 border border-gray-200 rounded-md px-2 py-1.5 text-sm text-gray-900" /><button onClick={addLeaveType} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-md">Add</button></div>
          </div>
        </div>
      </div>
      ...
      {edit && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setEdit(null)}>
          <div className="bg-white border border-gray-200 rounded-lg w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-end gap-2 mt-5"><button onClick={() => setEdit(null)} className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Cancel</button><button onClick={saveEdit} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium rounded-md">Save</button></div>
          </div>
        </div>
      )}
    </div>
