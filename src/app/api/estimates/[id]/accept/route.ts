...
// POST /api/estimates/[id]/accept — staff-side accept (portal accept = Phase 7).
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  return apiHandler(async () => {
    ...
    if (!(await hasModuleAccess(est.workspaceId, session.user.id, "canAccessInvoicing", session.user.role)))
      return forbidden();
    if (!["sent", "viewed"].includes(est.status)) return conflict("Only sent estimates can be accepted");
