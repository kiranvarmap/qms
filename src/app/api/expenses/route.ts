    ...
    const expense = await db.transaction(async (tx) => {
      ...
      const [row] = await tx
        ...
        .values({
          ...
          paidThroughAccountId: input.paidThroughAccountId || null,
          expenseType: input.expenseType ?? "goods",
          sacCode: input.sacCode || null,
          gstTreatment: input.gstTreatment || null,
          sourceOfSupply: input.sourceOfSupply || null,
          destinationOfSupply: input.destinationOfSupply || null,
          reverseCharge: input.reverseCharge ?? false,
          taxRateId: input.taxRateId || null,
          taxInclusive: input.taxInclusive ?? false,
          invoiceNumber: input.invoiceNumber || null,
          reportingTags: input.reportingTags ?? {},
