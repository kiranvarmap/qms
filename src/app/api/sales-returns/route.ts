...
import { createSalesReturnSchema } from "@/lib/validations";
import { listSalesReturns, createSalesReturn } from "@/lib/services/sales-extended";
...
export async function GET(req: Request) {
  return apiHandler(async () => {
    ...
    return ok({ data: await listSalesReturns(workspaceId) });
...
export async function POST(req: Request) {
  return apiHandler(async () => {
    const session = await auth();
    if (!session) return unauthorized();
    const input = createSalesReturnSchema.parse(await req.json());
    if (!(await hasModuleAccess(input.workspaceId, session.user.id, "canAccessInvoicing", session.user.role))) return forbidden();
    return created(await createSalesReturn(input.workspaceId, input, session.user.id));
